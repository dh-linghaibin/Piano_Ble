...
void USBH_Process(USB_OTG_CORE_HANDLE *pdev , USBH_HOST *phost)
{
  ...
  //查看Host端口事件
  /* check for Host port events */   //(HCD_IsDeviceConnected(pdev) == 0)属于硬件层
  if ((HCD_IsDeviceConnected(pdev) == 0)&& (phost->gState != HOST_IDLE)) 
  {
    ...
    /*********************************判断状态机的各种状态*******************************/   
  switch (phost->gState) /*  Host State Machine Value */ //HOST状态机的值
  {
  /********************************主机空闲状态*******************************/   
  case HOST_IDLE :
    ...
    break;
   /*********************************主机连接状态*******************************/    
  case HOST_DEV_ATTACHED :   //1 设备连着  //printf("Device Attached\n"); //设备已经连接
    
    phost->usr_cb->DeviceAttached(); //DeviceAttached()函数中获得连接值 在usbh_usr.c中定义 
    phost->Control.hc_num_out = USBH_Alloc_Channel(pdev, 0x00);    //分配输出通道地址           //给USB终端设备分配的传出通道 
    phost->Control.hc_num_in = USBH_Alloc_Channel(pdev, 0x80);   //分配输入通道地址    //分配USB设备的传入通道
  
    /* Reset USB Device */
	 //HCD_ResetPort(pdev) == 0 属于硬件层            
    if ( HCD_ResetPort(pdev) == 0) //复位USB设备  //如果 检测到 复位命令
    {
      phost->usr_cb->ResetDevice();//打印“ResetDevice”  //那么执行 USB复位，不是主机复位！
      /*  Wait for USB USBH_ISR_PrtEnDisableChange()  
      Host is Now ready to start the Enumeration 
      */
         //主机现在准备开始枚举
      phost->device_prop.speed = HCD_GetCurrentSpeed(pdev);  //获取当前设备的速度给device_prop结构体
      
      phost->gState = HOST_ENUMERATION; //枚举（把phost结构体里的gState主机状态设置为枚举）
      phost->usr_cb->DeviceSpeedDetected(phost->device_prop.speed); //打印高速设备还是低速设备或者是全速设备
        
      /* Open Control pipes */
			   //打开控制管道
          //输入通道
      USBH_Open_Channel (pdev,  //设备      
                           phost->Control.hc_num_in,  //分配输出通道的地址
                           phost->device_prop.address,  //地址
                           phost->device_prop.speed,  //速度
                           EP_TYPE_CTRL,      //端点类型为控制端点
                           phost->Control.ep0size); //phost->Control.ep0size 控制端点0初始值64
  ...
  case HOST_ENUMERATION:     
    /* Check for enumeration status */  
	 //USBH_HandleEnum(pdev , phost)完整的处理了枚举（重点） 
    if ( USBH_HandleEnum(pdev , phost) == USBH_OK) //查看枚举状态，进入枚举阶段
    { 
      /* The function shall return USBH_OK when full enumeration is complete */
      
      /* user callback for end of device basic enumeration */
			  //当全部枚举完成时，这个函数将会返回USBH_OK
			 //设备基本枚举完成
      phost->usr_cb->EnumerationDone();    //printf("Enumeration completed\n");  //显示枚举完成
      
      phost->gState  = HOST_USR_INPUT;     //Host State Machine Value = 9   //用户按下按键
    }
    break;
    /*********************************用户按下状态（初始化）*******************************/     
  case HOST_USR_INPUT:    
    /*The function should return user response true to move to class state */
	//该函数应该返回真正的移动用户的响应类的状态
    //if ( phost->usr_cb->UserInput() == USBH_USR_RESP_OK)//检测按键是否按下
   // {
			//Init()函数原型是USBH_HID_InterfaceInit(),在usbh_hid_core.c中定义HID 接口初始化
         //USBH_HID_InterfaceInit()主要做一些类请求的初始化，方便下面的类请求USBH_HID_ClassRequest()
      if((phost->class_cb->Init(pdev, phost))\
        == USBH_OK) //如果按下(注意，目前这个状态初始化完成不了)
      {
        phost->gState  = HOST_CLASS_REQUEST;     //如果按下则响应主机类    
      }     
    //}   
    break;
    /*********************************主机类请求状态机*******************************/     
  case HOST_CLASS_REQUEST:  
    ...
    status = phost->class_cb->Requests(pdev, phost); //Requests()函数原型是USBH_HID_ClassRequest()
    
     if(status == USBH_OK)
     {
       phost->gState  = HOST_CLASS; //标准类响应完成
     }  
     
     else
     {
       USBH_ErrorHandle(phost, status);   //这个函数处理主机端的错误
